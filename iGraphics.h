...
/*
 *
 *Student Modification:Najmus Sakib Rashid
 *
 */
...
void iShowBMP2(int x, int y, char filename[], int ignoreColor)
{
    ...
    int i,j,k,c_x,c_y,r_w=0,r_h=0,m_i=0,mean;
    if(x<0)
        r_w=-x;
    if(y<0)
        r_h=-y;
    ...
    for (i = 0, j=0; i < nPixels; i++, j += 3)
    {
        ...
        {
            int rgb = 0;
//            if(/*chkbit(x+c_x-ox,y+c_y-oy||chkbitcar(0,x+c_x-ox,y+c_y-oy)||chkbitcar(1,x+c_x-ox,y+c_y-oy)||chkbitcar(2,x+c_x-ox,y+c_y-oy)||*/chkbitcar(3,x+c_x-ox,y+c_y-oy)){
//                mean=(TextureImage->data[j]+TextureImage->data[j+1]+TextureImage->data[j+2])/3;
//                for(int k = 2; k >= 0; k--)
//                {
//                    rgb = ((rgb << 8) | mean);
//                }
...
//            else
            ...
            rgPixels[m_i] = (rgb == ignoreColor) ? 0 : 255;
            rgPixels[m_i] = ((rgPixels[m_i] << 24) | rgb);
            m_i++;
        }
    }

    glRasterPos2f(x+r_w, y+r_h);
    glDrawPixels(width-r_w, height-r_h, GL_RGBA, GL_UNSIGNED_BYTE, rgPixels);

    delete []rgPixels;
    free(TextureImage->data);
    free(TextureImage);
}
bool iShowBMP2custom(int x, int y, char filename[], int p_x,int p_y,int w,int h, int ignoreColor,int r,int g,int b,int *ind,int col,int *fr)
{
    AUX_RGBImageRec *TextureImage;
    TextureImage = auxDIBImageLoad(filename);

    int i,j,k,c_x,c_y,r_w=0,r_h=0,m_i=0;
    bool ret=0;
    if(x<0)
        r_w=-x;
    if(y<0)
    ...
    *fr=0;
    for (i = 0, j=0; i < nPixels; i++, j += 3)
    {
        c_x=i%width;
        c_y=i/width;
        if(c_x>=r_w+p_x&&c_x<p_x+w&&c_y>=r_h+p_y&&c_y<p_y+h)
        {
            int rgb = 0;
            if(!r)
                TextureImage->data[j]=(TextureImage->data[j]==255)?255:0;
            if(!g)
                TextureImage->data[j+1]=(TextureImage->data[j+1]==255)?255:0;
            if(!b)
                TextureImage->data[j+2]=(TextureImage->data[j+2]==255)?255:0;
            for(int k = 2; k >= 0; k--)
            {
                rgb = ((rgb << 8) | TextureImage->data[j+k]);
            }

            if(rgb!=ignoreColor&&chkbit(x+c_x-ox-p_x,y+c_y-oy-p_y))
            {
                ret=1;
            }
            for(int i=0; i<4; i++)
            {
                if(rgb!=ignoreColor&&chkbitcar(i,x+c_x-ox-p_x,y+c_y-oy-p_y))
                {
                    if(ret==0)
                    {
                        *ind=i;
            ...
            if(rgb!=ignoreColor&&chkbitcar(4,x+c_x-ox-p_x,y+c_y-oy-p_y))
            {
                *fr=1;
            }
            if(col&&rgb!=ignoreColor)
                onbitcar(r,x+c_x-ox-p_x,y+c_y-oy-p_y);
    ...
    glRasterPos2f(x+r_w, y+r_h);
    glDrawPixels(w-r_w, h-r_h, GL_RGBA, GL_UNSIGNED_BYTE, rgPixels);
...
void iShowBMP2whiteblack(int x, int y, char filename[], int ignoreColor)
{
    AUX_RGBImageRec *TextureImage;
    TextureImage = auxDIBImageLoad(filename);
    ...
    for (i = 0, j=0; i < nPixels; i++, j += 3)
    {
        ...
        {
            int rgb = 0;
//            mean=TextureImage->data[j+2];
//            TextureImage->data[j+2]=TextureImage->data[j+1];
//            TextureImage->data[j+1]=TextureImage->data[j+0];
//            TextureImage->data[j+0]=mean;
//            TextureImage->data[j]=0;
//            TextureImage->data[j+1]=0;
            if(TextureImage->data[j+2]>200)
            {
                TextureImage->data[j]=0;
                TextureImage->data[j+1]=0;
                TextureImage->data[j+2]=255;
            }
            for(int k = 2; k >= 0; k--)
            {
                rgb = ((rgb << 8) | (TextureImage->data[j+k]));
